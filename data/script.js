...
async function loadJsonData(pageName) {
  try {
    ...
    if (pageName === '/') {
      ...
      isIndexPage = false;
      return; // для index НЕ нужны теги/фильтры
    }

    // страницы Windows, Android...
    const pageData = jsonData?.data[pageName];
    if (!pageData) return createPageTagsFilter(pageName);
...
function createPageTagsFilter(pageName) {
  ...
  const tagsData = window?.jsonData?.TAGS[pageName] == undefined ? GetSearchTags() : window?.jsonData?.TAGS[pageName]
